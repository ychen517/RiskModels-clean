...
class SimpleStandardization:
    ...
    def __init__(self, exceptionNames=None, 
                 ...
                 fillWithZeroList=[], forceMAD=False, 
                 ):
    ...
    def standardize(self, expM, estu, mcaps, date, writeStats=True,eligibleEstu=[],noClip_List=[]):
        """Standardize factors.
        """

        styleIndices = self.preProcessExposureData(expM, estu,noClip_List)
        assetIndices = list(range(len(expM.getAssets())))
        self.standardizeInternal(styleIndices, assetIndices, expM, estu, mcaps, writeStats)

    def preProcessExposureData(self, expM, estu,noClip_List=[]):
        ...
        if len(styleIndices) == 0:
            if not self.forceMAD:
                return []
            else:
                styleIndices = [s for s in expM.getFactorIndices(ExposureMatrix.StyleFactor) \
                                if s not in dummyIndices]
        ...
        fac_names = expM.getFactorNames()
        for (i,j) in enumerate(styleIndices):
            if fac_names[j] in noClip_List:
                logging.warning('Factor %s is in the noClip List, using unclipped values',fac_names[j])
            else:
    ...
    def standardizeInternal(self, factorIndices, assetIndices, expM, 
        ...
        avg = Utilities.screen_data(avg, fill=True)
        ...
        if self.debuggingReporting:
            ...
            if bucketName is None:
                ...
                logging.info('Weighted average for bucket %s: %s',
                        bucketName, ','.join(['%f' % n for n in numpy.dot(numpy.transpose(tmp_data), tmp_wgt)]))
                logging.info('Standard deviation for bucket %s: %s',
                        bucketName, ','.join(['%f' % n for n in [numpy.inner(tmp_data[:,i], tmp_data[:,i]) / (len(indices)-1) for i in range(len(factorIndices))]]))
        ...
        if writeStats:
            ...
            if not hasattr(expM, 'stdDict'):
                expM.stdDict = {}
            if bucketName in expM.meanDict:
                expM.meanDict[bucketName].update(meanDict)  # update volatility which is standardized twice
            else:
                expM.meanDict[bucketName] = meanDict
            if bucketName in expM.stdDict:
                expM.stdDict[bucketName].update(stdDict)
            else:
                expM.stdDict[bucketName] = stdDict
...
class BucketizedStandardization(SimpleStandardization):
    ...
    def standardize(self, expM, estu, mcaps, date, writeStats=True, eligibleEstu=[], noClip_List=[]):
        """Standardize factors.
        """
        styleIndices = set(self.preProcessExposureData(expM, estu,noClip_List))
