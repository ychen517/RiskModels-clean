...
from lib.common.assetAddtitionNonDjango import runAssetAdditionCommands
...
from marketdb.Utilities import Struct as marketdbStruct
...
def runJsonCommands(commandsJsonFileName,jobid=None,testOnlyParam=False):
    ...
    try:
        if (commandsJsonFileName.find("macCurvesHistory")>-1):
            ...
            (errors, textresult, commands) = runAssetAdditionCommandsETF(
            ...
            results= runAssetAdditionCommands(
                            commands.get('sid',None),
                            commands.get('assetAdditionCommands',[]),
                            commands.get('qaReportsCommands',[])
                            )
            if len(results.errors) > 0:
                logger.error("Error during assetAddition runJsonCommands(): %s",results.errors)
            logger.debug("textresult in runJsonCommands(): %s",results.textresult)    
            return  (results.errors, results.textresult, results.commands)
...
def runAssetAdditionCommandsETF(sid, assetAdditionCommands,qaReportsCommands=[], macdbId = None, jobid=None):
    logger.setLevel(logging.DEBUG)
    logger.debug('in runAssetAdditionCommands() %s %s',sid, assetAdditionCommands)
...
#     rval.errors = errors
#     rval.commands=commands
#     rval.result=result
#     rval.textresult = textresult
#     rval.marketIdList = marketIdList
#     rval.marketIdsStr = ','.join(marketIdList)
                elif isinstance(result, marketdbStruct):
                    logger.info('allMarketIdStr1: %s ', result.marketIdsStr)
                    
                elif len(result) == 3 and len(result[1])>0:
                    logger.info('allMarketIdStr2: %s ',  result[1][-1]   )
