...
export p_year=$year
export prog="python3 generateFundamentalModel.py $LAZY_GL -m$MD --force"
echo "####################################################################################################################" >> tmp.sh
echo "# ESTU" >> tmp.sh
while [ $year -le $end ]
do
...
while [ $year -le $end ]
do
    export group=${MD}_exp_$year
    export depend="${MD}_part_estu_$year,${MD}_estu_$p_year"
    echo "$runit --cmd='$prog -m$MD --exposures $year-01-01 $year-12-31' --group=$group --depends-on=$depend run" >> tmp.sh
    export p_year=$year
...
while [ $year -le $end ]
do
    export group=${MD}_ret_$year
    export depend="${MD}_exp_$year,${MD}_exp_$p_year"
    echo "$runit --cmd='$prog -m$MD --factors $year-01-01 $year-12-31' --group=$group --depends-on=$depend run" >> tmp.sh
    export p_year=$year
...
do
    ...
    echo "$runit --cmd='$prog -m$MD --risks $year-01-01 $year-12-31' --group=$group --depends-on=$depend run" >> tmp.sh
